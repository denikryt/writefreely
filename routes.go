...
func InitRoutes(apper Apper, r *mux.Router) *mux.Router {
	...
	apiColls.HandleFunc("/{alias}/posts/{post:[a-zA-Z0-9]{10}}/categories", handler.All(assignPostCategories)).Methods("POST")
	...
	apiColls.HandleFunc("/{alias}/categories/{slug}", handler.All(updateCollectionCategory)).Methods("POST", "PUT")
...
func RouteCollections(handler *Handler, r *mux.Router) {
	...
	r.HandleFunc("/category/{slug}", handler.Web(handleViewCategory, UserLevelReader))
	r.HandleFunc("/category/{slug}/page/{page:[0-9]+}", handler.Web(handleViewCategory, UserLevelReader))
	r.HandleFunc("/category/{slug}/feed/", handler.Web(ViewFeed, UserLevelReader))
